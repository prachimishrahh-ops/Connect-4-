...
(async () => {
    console.log('\n🔧 TESTING MOVE 7 FIX - State Refresh + Retry Logic\n');
    ...
    await pageA.fill('#playerName', 'FixTestRed');
    await pageB.fill('#playerName', 'FixTestYellow');
    ...
    console.log('━'.repeat(70));
    console.log('GAMEPLAY PHASE - Testing Move 7 Fix');
    console.log('━'.repeat(70) + '\n');
    ...
    for (let i = 0; i < moves.length; i++) {
        ...
        // Wait longer for winning move
        const waitTime = isWinning ? 15000 : 6000;
        console.log(`⏳ Waiting ${waitTime/1000}s for processing...\n`);
        ...
        if (discsA === i + 1 && discsB === i + 1) {
            ...
            if (isWinning) {
                console.log(`\n❌ MOVE 7 FAILED - Fix did not work!`);
            }
        }
    }
    ...
    await pageA.screenshot({ path: 'move7-fix-test-red.png', fullPage: true });
    await pageB.screenshot({ path: 'move7-fix-test-yellow.png', fullPage: true });
    console.log('\n📸 Screenshots saved\n');
