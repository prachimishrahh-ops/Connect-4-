...
impl Contract for BankrollContract {
    ...
    type InstantiationArgument = BankrollParameters;
    type EventValue = ();
    ...
    async fn instantiate(&mut self, argument: Self::InstantiationArgument) {
        // Store application parameters in state to avoid runtime.application_parameters() calls
        // which cause panic in Linera 0.15.7
        self.state.master_chain.set(Some(argument.master_chain));
        self.state.bonus_amount.set(Some(argument.bonus));
    }
        ...
        match operation {
            ...
            BankrollOperation::Balance { owner } => {
                ...
                if daily_bonus.is_zero() {
                    daily_bonus.update_bonus(self.state.bonus_amount.get().expect("bonus_amount not initialized"));
                }
            ...
            BankrollOperation::MintToken { chain_id, amount } => {
                ...
                assert_eq!(
                    ...
                    self.state.master_chain.get().expect("master_chain not initialized"),
    ...
    async fn execute_message(&mut self, message: Self::Message) {
        ...
        match message {
            ...
            BankrollMessage::DebtNotif { debt_id, amount, created_at } => {
                ...
                let master_chain = self.state.master_chain.get().expect("master_chain not initialized");
